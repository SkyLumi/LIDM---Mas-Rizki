export class Game extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.image(width / 2, 0, 'machinery');

        this.cone = this.physics.add.sprite(width / 2, height - 200, 'cone');
        this.cone.body.setAllowGravity(false);
        this.cone.setSize(80, 50); // Sesuaikan hitbox untuk tabrakan
        
        // Buat grup untuk es krim
        ...
        // 2. Setup Fisika (Phaser)
        // Panggil this.handleCatch saat cone overlap dengan es krim
        this.physics.add.overlap(this.cone, this.iceCreams, this.handleCatch, null, this);

        // 3. Setup MediaPipe
        this.targetConePos = { x: width / 2, y: height - 100 }; // Posisi awal
        this.initMediaPipe();

        // 4. Setup UI (Skor, Nyawa)
        // ... (this.add.text(...), this.add.image(...) untuk UI) ...
