import MediaPipeManager from "./mediapipeManager.js";
   ...
   export default class game extends Phaser.Scene {
      ...
      pauseGame() {
         // Jangan pause kalo udah di-pause atau board-nya lagi kekunci
         ...
         // Sembunyiin kursor tangan & tombol pause
         ...
         // Tambahkan pausePanel kamu
         const panel = this.add.image(180, 30, 'pausePanel').setScale(1.5);

         // --- PERUBAHAN URUTAN DI SINI ---
         
         const buttonY = 150; // Atur Y biar pas sama panel
         const buttonSpacing = 150; // Atur jarak antar tombol

         // KIRI: (x = -spacing, y = buttonY)
         ...
         // Masukin semua ke container
         this.pauseContainer.add([bg, panel, continueBtn, retryBtn, homeBtn]);

         // --- BATAS PERUBAHAN ---


         // --- Kasih fungsi ke tombol (ini tidak berubah) ---
      ...
      resumeGame() {
         ...
         this.isPaused = false;
         this.isBoardLocked = false; // Buka lagi kuncinya

         // Hancurin panel pause
         ...
         // Munculin lagi kursor tangan
         if (this.handCursor) {
            this.handCursor.setVisible(true);
         }
         
         // Munculin lagi tombol pause
      ...
      endGame(isWin, message) {
         ...
         // 2. Cek bintang yang udah ada di registry
         //    (Pakai || 0 untuk jaga-jaga kalau datanya belum ada)
         const currentSavedStars = this.registry.get(storageKey) || 0;

         // 3. Simpan HANYA JIKA bintang baru > bintang lama
         if (stars > currentSavedStars) {
            this.registry.set(storageKey, stars);
            // console.log(`Registry set: ${storageKey} = ${stars}`); // Opsional buat nge-cek
         }
